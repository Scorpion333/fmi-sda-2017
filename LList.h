...
public:
    ...
    bool hasElement(const T& x) {
        return hasElement(x, start);
    }
    
    bool hasElement(const T& x, elem_link<T>* _start) {
        elem_link<T>* current = _start;             // This is new 'current', not the 'current' on line 16!
        while (current != NULL) {
            if (current->inf == x)
                return true;
    ...
    bool hasDuplicate() {
        elem_link<T>* current = start;
        while (current != NULL) {
            if (hasElement(current->inf, current->link)) {
                return true;
            }
            current = current->link;
        }
